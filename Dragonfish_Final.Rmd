...
stom.tree <- ape::read.tree("dragonfish.tree")
...
stom.sp <- read_csv("stom_image_data.csv")
head(stom.sp)
head(stom.sp$identifier)

#
out.data <- tibble(xy.file=basename(names(outs))) %>% 
  mutate(identifier=gsub(".txt","",xy.file)) %>% 
  left_join(stom.sp)
...
***Figure 7. Adjusted phylogenetic tree of stomidoptera included in the study***
...
```{r, include= FALSE}
library(RRphylo)
hindPC1.RR <- RRphylo(tree=stom.tree2,y=hind.pc1)
hindPC1.SS<- search.shift(RR=hindPC1.RR,status.type="clade")
forePC1.RR <- RRphylo(tree=stom.tree2,y=fore.pc1)
