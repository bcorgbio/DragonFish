...
#add species info
out.df <- out.df %>% 
  mutate(species=gsub("\\..+","\\1",basename(filename))) %>% 
  na.omit()
#make list
outs.l <- sapply(f,function(x) out.df %>% filter(filename==x) %>% select(X,Y) %>% as.matrix)
#extract skull info
wings <- gsub("\\..+","\\1",basename(names(outs.l)))
outs <-  outs.l %>% 
...
***Figure 2. Visualization of Skull using EFA after the size of the skull have been equalized***
